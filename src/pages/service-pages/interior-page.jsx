import React from "react";
import interior1 from "../../assets/images/interiorimages/interior1.jpg";
import interior2 from "../../assets/images/interiorimages/interior2.jpg";
import interior3 from "../../assets/images/interiorimages/interior3.jpg";
import interior4 from "../../assets/images/interiorimages/interior4.jpg";
import interior5 from "../../assets/images/interiorimages/interior5.jpg";
import interior6 from "../../assets/images/interiorimages/interior6.jpg";
import interior7 from "../../assets/images/interiorimages/interior7.jpg";
import interior8 from "../../assets/images/interiorimages/interior8.jpg";
import interior9 from "../../assets/images/interiorimages/interior9.jpg";

const Interiorpage = () => {
  return (
    <div className='min-h-screen bg-slate-500'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col justify-center items-center'>
        <h1 className='text-4xl text-slate font-bold mb-4'>Interior Design</h1>
        {/* <h3 className="text-xl text-slate">Our Exclusive Services We Offer</h3> */}
      </div>

      <div className='max-w-screen-2xl mx-auto px-4 md:px-8 py-8'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior1} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              SPORTS CLUB INTERIOR
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior2} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior3} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior4} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior5} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior6} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior7} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior8} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
          <div className='bg-white p-4 shadow-md rounded-lg'>
            <img src={interior9} alt='' className='mb-4 rounded-lg' />
            <h2 className='text-xl font-bold text-slate-800 mb-2'>
              INTERIOR DESIGN
            </h2>
            <p className='text-slate-600'>
              Using latest technology to make efficient use of spaces, Better
              designing concepts.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Interiorpage;
