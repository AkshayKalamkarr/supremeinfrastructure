...
const HeroSection = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      {/* Logo Positioning */}
      <motion.div 
        className="absolute top-4 left-4 md:top-4 md:left-20"
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        <img src="/images/logo.png" alt="Logo" className="h-10 md:h-24" />
      </motion.div>
      ...
      <div className="absolute inset-0 bg-black bg-opacity-30 flex flex-col items-start justify-center p-4 sm:p-8 md:p-12 lg:p-16">
        ...
        <motion.button 
          className="mt-6 px-6 py-2 bg-red-600 text-white font-semibold rounded-full hover:bg-red-700 transition-colors duration-300 text-sm sm:text-xl md:ml-14"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
